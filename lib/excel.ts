...
import { getStartingRow } from './utils/file-utils';
...
export function mergeExcelFiles(files: ExcelFile[]): XLSX.WorkBook {
  ...
  try {
    ...
    baseFile.sheetNames.forEach((sheetName, index) => {
      const mergedData: any[][] = XLSX.utils.sheet_to_json(baseFile.sheets[index], {
        header: 1,
      });
      ...
      for (let i = 1; i < files.length; i++) {
        ...
        if (index < currentFile.sheets.length) {
          ...
          const currentData: any[][] = XLSX.utils.sheet_to_json(currentSheet, {
            header: 1,
          });
